...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) {
      throw new AppError(`Could not find any customer with id:${customer_id}`);
    }
    ...
    if (!existentProducts.length) {
      throw new AppError(
        "Could not find any products with given id's provided",
      );
    }
    ...
    const OrdersProductsQuantity = products.map(product => ({
      id: product.id,
      quantity:
        existentProducts.filter(p => p.id === product.id)[0].quantity -
        product.quantity,
